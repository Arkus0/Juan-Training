...
void main() async {
  ...
  if (!await inputFile.exists()) {
    print('❌ No se encontró assets/data/exercises.json');
    exit(1);
  }
  ...
  print('📊 Ejercicios originales: ${allExercises.length}');
  
  // Crear backup
  await backupFile.writeAsString(content);
  print('💾 Backup creado en: ${backupFile.path}');
  ...
  print('✂️ Ejercicios después de podar: ${prunedExercises.length}');
  ...
  if (missingIds.isNotEmpty) {
    print('⚠️ IDs no encontrados: $missingIds');
  }
  
  // Ordenar por grupo muscular y nombre
  prunedExercises.sort((a, b) {
    final groupCompare = (a['muscleGroup'] as String).compareTo(b['muscleGroup'] as String);
  ...
  if (await alternativasFile.exists()) {
    ...
    await File('assets/data/alternativas_backup.json').writeAsString(altContent);
    ...
    for (final entry in alternativas.entries) {
      ...
      final List<dynamic> originalAlts = entry.value as List<dynamic>;
      final filteredAlts = originalAlts.where((altId) => foundIds.contains(altId)).toList();
    ...
    await File('assets/data/alternativas_pruned.json').writeAsString(altEncoder.convert(prunedAlternativas));
    
    print('\n📋 Alternativas:');
    print('   Claves originales: ${alternativas.length}');
    print('   Claves después de podar: ${prunedAlternativas.length}');
    print('   Claves eliminadas: $removedKeys');
    print('   Referencias eliminadas: $removedValues');
  }
  
  print('\n🎯 Para aplicar los cambios:');
  print('   1. Revisar assets/data/exercises_pruned.json');
  print('   2. Revisar assets/data/alternativas_pruned.json');
  print('   3. Si está bien, ejecutar:');
  print('      copy assets\\data\\exercises_pruned.json assets\\data\\exercises.json');
  print('      copy assets\\data\\alternativas_pruned.json assets\\data\\alternativas.json');
}
