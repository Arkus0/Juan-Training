...
Future<void> main() async {
  ...
  final endpoints = [
    ...
    (int id, int lang) =>
        'https://wger.de/api/v2/exercise/$id/?language=\$lang',
    (int id, int lang) =>
        'https://wger.de/api/v2/exerciseinfo/$id/?language=\$lang',
  ];
  for (final id in ids) {
    stdout.writeln('\n=== ID $id ===');
        ...
        try {
          ...
          if (r.statusCode == 200 && r.body.isNotEmpty) {
            try {
              ...
              final name = j is Map && j.containsKey('name')
                  ? j['name']
                  : (j is Map && j.containsKey('exercise')
                      ? j['exercise']
                      : null);
              final desc = j is Map && j.containsKey('description')
                  ? ((j['description'] as String?)?.replaceAll('\n', ' ') ?? '')
                  : '';
              stdout.write(' | name=$name');
              if (desc.isNotEmpty) {
                stdout.write(
                    ' | desc=${desc.substring(0, desc.length > 80 ? 80 : desc.length)}',);
              }
            } catch (e) {
