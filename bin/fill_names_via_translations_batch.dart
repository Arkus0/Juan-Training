...
Future<String?> _fetchNameViaTranslations(int id) async {
  ...
  try {
    ...
    final infoResp =
        await http.get(Uri.parse(infoUrl)).timeout(const Duration(seconds: 8));
    ...
    if (transId != null) {
      ...
      final exResp =
          await http.get(Uri.parse(exUrl)).timeout(const Duration(seconds: 8));
...
Future<void> main(List<String> args) async {
  ...
  while (processed < missingIdx.length) {
    final end = (processed + batchSize) < missingIdx.length
        ? (processed + batchSize)
        : missingIdx.length;
    ...
    if (updatedThisBatch > 0) {
      ...
      stdout
          .writeln('Wrote JSON; updated $updatedThisBatch names in this batch');
    } else {
