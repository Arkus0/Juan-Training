...
final allProgressionDecisionsProvider =
    Provider<Map<int, ProgressionDecision>>((ref) {
  final state = ref.watch(trainingSessionProvider);
...
final sessionProgressionSummaryProvider =
    Provider<SessionProgressionSummary>((ref) {
  final decisions = ref.watch(allProgressionDecisionsProvider);
...
ExerciseProgressionContext _buildContextFromHistory({
  ...
  final consecutiveFailures = (sessionResult == SessionResult.partial ||
...
final exerciseEmpatheticFeedbackProvider =
    Provider.family<EmpatheticFeedbackResult, int>(
  (ref, exerciseIndex) {
    final state = ref.watch(trainingSessionProvider);
    ...
    final failedSets = exercise.logs
        .where(
          (log) =>
              log.completed && log.reps < 6, // Asumiendo que <6 reps es "fallo"
        )
        .length;
...
final exerciseEmpatheticBannerProvider = Provider.family<String?, int>(
  (ref, exerciseIndex) {
    final feedback =
        ref.watch(exerciseEmpatheticFeedbackProvider(exerciseIndex));
